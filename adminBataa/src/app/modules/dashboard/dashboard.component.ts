...
export class DashboardComponent implements OnInit {
  ...
  constructor(private UT : UserService,private UTT : UtilisateurService,private CT : CategorieService , private SCT : SouscategorieService , private PT : ProduitService,private datePipe: DatePipe) {this.dateAujourdui=new Date(); }
  ...
  getproduitbycategorie(){
    ...
    this.dateform = this.datePipe.transform(this.dateAujourdui, 'yyyy-MM-dd');
    this.PT.getProduitValider().subscribe(x=>{
      let nbcat1 =0;
      let nbcat2 =0;
      let nbcat3 =0;
      x.forEach(element => {
        if(element.sous_categories.categorieID==this.idcategories[0] && element.etat_vente != 1 && element.date_expiration>this.dateform)
          nbcat1++;
        if(element.sous_categories.categorieID==this.idcategories[1] && element.etat_vente != 1 && element.date_expiration>this.dateform)
          nbcat2++;
        if(element.sous_categories.categorieID==this.idcategories[2] && element.etat_vente != 1 && element.date_expiration>this.dateform)
          nbcat3++
